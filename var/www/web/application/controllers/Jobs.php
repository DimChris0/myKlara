...
class Jobs extends CI_Controller 
{
    ...
    public function add_job()
    {
    	...
    	$job_type	    = $this->input->post('operation_option');


		$scan_fileset = $yara_fileset_scan[0];
		$this->db->where('entry', $scan_fileset);
		$q = $this->db->get('scan_filesets');
		if ($q->num_rows() != 1)
		{
			$sql = "insert into scan_filesets (entry) values (?)";
			$this->db->query($sql, $scan_fileset);
			$this->db->where('entry', $scan_fileset);
			$q = $this->db->get('scan_filesets');
		        $set = $q->result_array();
		        $yara_fileset_id = $set[0]['id'];
		}
		else
		{
			$set = $q->result_array();
	...
	// Return status from internal function
        echo $this->global_functions->jobs_add($yara_rules, $yara_fileset_id, $job_type);
    }
