...
def yara_scan(scan_options):
    ...
    try:
        ...
        yara_rules_temp_file_fn = (os.path.abspath(yara_rules_temp_file.name)).replace('\\', '/')
    ...
    yara_cmd += (str(os.path.abspath(yara_rules_temp_file.name))).replace('\\', '/') + " "
...
def list_files_and_dirs(scan_options):
    assert (isinstance (scan_options, dict))
    assert (isinstance (config.head_path_and_args, list))
    ...
    time_start = int (time.time())
    dirlist = ""
    for x in os.listdir(str(fileset_scan)):
        if os.path.isfile(str(fileset_scan) + x): dirlist += 'file-'+ x + '\n'
        elif os.path.isdir(str(fileset_scan) + x): dirlist += 'dir -' + x + '\n'
        elif os.path.islink(str(fileset_scan) + x): dirlist += 'link-' + x + '\n'
        else: dirlist += '-unknown-' + x + '\n'

    # Setting up the results dict
    time_end = int (time.time())
    results['finish_time'] = time.strftime('%Y-%m-%d %H:%M:%S')
    results['execution_time'] = time_end - time_start
    results['yara_results'] = dirlist

    return results



def md5_of_files(scan_options):
    assert (isinstance (scan_options, dict))
    assert (isinstance (config.head_path_and_args, list))
    ...
    hashes = ""
    for x in os.listdir(str(fileset_scan)):
        if os.path.isfile(str(fileset_scan) + x): hashes += 'file '+ x + ' has MD5: ' + hashlib.md5(open(str(fileset_scan) + x, 'rb').read()).hexdigest() + '\n'

    # Setting up the results dict
    time_end = int (time.time())
    results['finish_time'] = time.strftime('%Y-%m-%d %H:%M:%S')
    results['execution_time'] = time_end - time_start
    results['yara_results'] = hashes
...
def extract_matched_files(yara_results, job_type):
    ...
    if job_type == 2:
        yara_res = ""
        for i in yara_results:
            yara_res += " " + i + "\n"
        return re.findall(pattern_for_yara_results, yara_res)
